...
namespace jd
{
	...
	class SphereComponent final : public StaticComponent
	{
	public:
		using delegate_t = Delegate<void, const event::PosEvent&>;

		SphereComponent(size_t instanceCount, const ShaderResoures& resource);
		void initilize(init_func_t init);

		void onUpdate(double deltaTime) override;
		void onRotate(const event::PosEvent& ev);

		delegate_t& getSubscribers() noexcept { return subscriber_; }
